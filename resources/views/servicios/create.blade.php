          ...
          <div class="card-body">
            <form action="{{ route('servicios.store') }}" method="POST">
              ...
              <div class="form-group">
                ...
                <input id="token" class="form-control{{ $errors->has('token') ? ' is-invalid' : '' }}" type="text" name="token" maxlength="80" value="{{ old('token') }}" placeholder="Token Wialon" readonly>
              </div>

              <div class="form-group">
                <button id="login-wialon" class="btn btn-primary btn-fill btn-block{{ old('token') ? ' d-none' : '' }}" type="button"{{ old('token') ? ' disabled' : '' }}>
                  Login con Wialon
                </button>

                <button id="remove-wialon" class="btn btn-danger btn-fill btn-sm{{ old('token') ? '' : ' d-none' }}" type="button"{{ old('token') ? '' : ' disabled' }}>
                  Eliminar token
                </button>
              </div>
              ...
              <div class="form-group text-right">
                ...
                <button id="send-form" class="btn btn-primary" type="submit" {{ old('token') ? '' : 'disabled' }}><i class="fa fa-send"></i> Guardar</button>
              </div>
            </form>
          </div>
        </div>
